    ...
    def update_k(self, k):
        ...
        for m in range(self.M):
            ...
            if self.likelihoods[m] in ['CTM']:
                ...
                # VI mu
                resid = self.y_node[m].data - np.ma.dot(self.w_node[m].E_w, self.E_z.T).T
                partial_resid = (resid + np.ma.outer(self.w_node[m].E_w[:,k], self.E_z[:,k]).T)
                first_term = np.ma.dot(self.w_node[m].E_w[:,k], self.tau_node[m].Sigma0_inv)
                vi_mu_new += np.ma.sum(first_term*partial_resid, axis=1)          
...
class nodeFA_w_m(nodeFA_general):  
    ...
    def ELBO(self):
        ...
        if self.W_priors[self.m] == "None":
            elbo = self.D[self.m]*self.K/2 - np.sum(self.E_w_squared)/2 + np.sum(log_eps(self.w_m_node_not_sparse.vi_var))
...
class nodeFA_tau_m(nodeFA_general):
    ...
    def __init__(self, a0, b0, m, general_params):
        ...
        if self.likelihoods[self.m] == 'CTM':
            self.Sigma0_inv = np.eye(self.D[m])
...
class FA():
    def __init__(self, data, 
        ...
        for m in range(self.M):
            ...
            if (self.W_priors[m] == 'ARD') or (self.W_priors[m] == 'ARD_SS'):
                ...
                node_alpha_m = nodeFA_alpha_m(1e-3, 1e-3, m, general_params=general_params)
            ...
            node_tau_m = nodeFA_tau_m(1e-3, 1e-3, m, general_params=general_params)
    ...
    def update(self):
        ...
        # update tau by m
        for m in range(self.M):
            if self.likelihoods in ['normal', 'Bernoulli']:
